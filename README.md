...
- 主界面以普通用户权限运行；点击执行优化或还原时，Windows 会针对该次操作请求管理员权限
...
自存方案和运行配置保存在 `%LocalAppData%\DeltaForceBooster`，还原备份保存在受保护的 `%ProgramData%\DeltaForceBooster\backup`，不会与程序文件混放。

### 预设方案

| 方案 | 说明 |
|---|---|
| `main` | 主推全套，覆盖主要系统、调度与显卡层设置；包含「显卡型号伪装」，GUI 会单独二次确认，CLI 需显式加 `-Risky` |
| `balanced` | 均衡推荐，保留桌面效果、鼠标手感、系统服务和休眠 |
| `safe-only` | 保守模式，仅修改当前用户设置，通常不需要重启；受保护备份仍会请求一次管理员权限 |
...
发布清单可声明最低支持版本。支持该字段的 v0.19.0 及以后客户端低于该值时，更新窗口不提供跳过或稍后选项，必须升级后才能继续使用；v0.20.1 修复了 v0.20.0 从旧安装目录更新时的身份误判，并将最低支持版本同步提升到 v0.20.1。更早客户端仍会收到普通更新提示，官网只提供最新版安装包。

用户点击「立即更新」后，工具会下载并校验更新包，再由短生命周期管理员 helper 复验并封存到受保护的暂存目录；安装器启动后还会再次校验，再完整解压、核对发布清单并事务切换版本。下载源限制为官方域名白名单，任一校验失败都不会安装。
更新安装前会等待发起更新的旧进程退出。旧版若仍装在下载文件夹等普通程序可写的位置，会自动迁移到 `%ProgramFiles%\DeltaForceBooster`，同时保留自存方案、备份和运行配置；切换失败会回滚旧版本。
...
- 经用户同意后发送匿名使用统计（随机安装标识、短期设备令牌、版本与硬件概况、启动/优化/还原汇总，以及游戏中最多 120 秒的平均帧率 / 1% 低帧率 / GPU 状态汇总和未使用/轻量/均衡/深度四档优化强度）。用户主动启动自动调优 Beta 后，还会发送实验编号、规则版本、固定候选组与项目 ID、每轮汇总指标、样本有效性及最终保留/回滚结果，用于评估规则效果；不上传自存方案、逐帧 CSV、用户名、机器名、SID、游戏路径、注册表内容或诊断日志，可在 `%LocalAppData%\DeltaForceBooster\config\telemetry.json` 中停用。汇总来自客户端自动采样，不是独立实验室测量；诊断报告保留 30 天、普通性能会话保留 90 天，匿名安装标识、按日使用明细和调优实验明细保留 180 天
- 诊断报告只有在用户主动点击并确认后才会上传
- 程序文件默认位于 Program Files；启动器内置关键文件哈希，GUI 只在具体系统操作时启动短生命周期管理员引擎
