...
- `pcie-check` / `vcredist-check` / `xmp-check` 是纯检测项，只读不写：分别报告 PCIe 链路
  ...
  若安装包运行后只显示「修复/卸载」，说明系统已有同版本或更高版本——选「修复」即可，
  修复后仍不一致才考虑在「应用和功能」里只卸载该架构的 VC++ 2015-2022 后重装，
  **绝不建议卸载其他年份的 VC++**（其他软件依赖）。
...
- 也可以整套套用预设方案：`-Apply -Preset balanced`。先用 `-ListPresets` 看可选方案，
  ...
  - `felix` 主推全套（30 项，界面显示为「★ 主推全套」且启动默认选中；Id 保持 felix
    不变，老命令与用户自存方案不受影响）：按抖音博主费利克斯Fx 的调试链路排列——
