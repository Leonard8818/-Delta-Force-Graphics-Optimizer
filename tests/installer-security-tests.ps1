...
Assert-True ($installerBuildSource -match "minimumSupportedVersion\s*=\s*'0\.22\.3'" -and
  "$($releaseManifest.version)" -eq '0.22.3' -and "$($releaseManifest.minimumSupportedVersion)" -eq '0.22.3') `
  'release policy must force every version below v0.22.3 to upgrade to v0.22.3'
...
$policyManifest = [ordered]@{
  version = '0.22.3'; minimumSupportedVersion = '0.22.3'; notes = '- test'
...
Set-BoosterSkipVersion '0.22.3' | Out-Null
...
Assert-True ($mandatoryUpdate -and $mandatoryUpdate.Mandatory -and $mandatoryUpdate.CanInline -and
  ...
  'v0.22.2 can skip or defer the mandatory v0.22.3 update'
Assert-True ($null -eq (Test-BoosterUpdate -CurrentVersion '0.22.3' -ManifestUrl ([Uri]$policyManifestPath).AbsoluteUri)) `
  'v0.22.3 incorrectly detects its own release as an update'
