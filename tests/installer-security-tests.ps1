...
Assert-True ($installerBuildSource -match "minimumSupportedVersion\s*=\s*'0\.22\.3'" -and
  "$($releaseManifest.version)" -eq '0.22.5' -and "$($releaseManifest.minimumSupportedVersion)" -eq '0.22.3') `
  'release policy must force every version below v0.22.3 to upgrade to the latest v0.22.5'
...
$policyManifest = [ordered]@{
  version = '0.22.5'; minimumSupportedVersion = '0.22.3'; notes = '- test'
...
Set-BoosterSkipVersion '0.22.5' | Out-Null
...
Assert-True ($mandatoryUpdate -and $mandatoryUpdate.Mandatory -and $mandatoryUpdate.CanInline -and
  ...
  'v0.22.2 can skip or defer the mandatory v0.22.5 update'
$optionalUpdate = Test-BoosterUpdate -CurrentVersion '0.22.3' -ManifestUrl ([Uri]$policyManifestPath).AbsoluteUri -IncludeSkipped
Assert-True ($optionalUpdate -and -not $optionalUpdate.Mandatory) 'v0.22.3 does not see v0.22.5 as an optional update'
Assert-True ($null -eq (Test-BoosterUpdate -CurrentVersion '0.22.5' -ManifestUrl ([Uri]$policyManifestPath).AbsoluteUri)) `
  'v0.22.5 incorrectly detects its own release as an update'
