...
## 一点五、安装版 vs 免安装版（绿色版）的真实区别

两个产物的 **payload 完全相同**（同一份 stage 目录压出来的，都含 uninstall.ps1 /
卸载.bat），功能没有任何差异，装好后也无法凭目录内容区分是哪个渠道来的。区别只在
获得方式与系统痕迹：

| | 安装版 Setup.exe | 免安装版 Portable.zip |
|---|---|---|
| 获得方式 | 图形向导（选位置、写入预检、按需提权） | 解压到任意目录即用 |
| 开始菜单/桌面快捷方式 | 向导按勾选项创建 | 不创建（用户可自己「发送到桌面快捷方式」） |
| 卸载入口 | 开始菜单「卸载优化助手」（勾选项开启时） | 无（直接删目录，或手动跑 卸载.bat） |
| 系统痕迹 | 快捷方式落点（无注册表卸载项） | 零痕迹，适合公司电脑等不想留安装记录的场景 |
| 升级方式 | 内置更新 → 安装器覆盖升级 | 同样走安装器：位置选到原目录即原地升级 |

是否保留免安装版由发布者决定（用户在议，暂保留）；删除它不影响安装版的任何功能。
