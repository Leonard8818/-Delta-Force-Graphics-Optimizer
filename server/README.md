...
软件通知使用三组接口：`GET /report/notifications` 向客户端公开最近 50 条仍有效的通知，允许缓存
15 秒且不包含任何管理凭据；`GET /api/notifications` 返回包含已撤回项的管理历史；
`POST /api/notifications` 发布标题、正文和 `info` / `important` / `warning` 级别，
`POST /api/notifications/{id}/withdraw` 撤回指定通知。后三个管理操作由 `/admin/api/*` 的
Basic Auth 和上游 `X-DFB-Admin-Token` 双层保护。客户端启动时读取一次，此后每 60 秒异步轮询，
本地仅缓存最近消息和最后已读 ID，用于断网时查看历史与恢复未读角标。
